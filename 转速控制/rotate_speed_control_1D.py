...
'''                第三步，使用当前测点集合进行预测泵转速                   '''
def predict_zhuansu(filename,testpoint,corr_tp_set):
    ...
    class client_net(object):
        ...
        def __init__(self):
            global net
            net = self.Net(n_feature=len(corr_tp_set), n_hidden=200, n_hidden1=200, n_output=1).to(device)
        ...
        def train_net(self,x,y,iter_max,acc):
            ...
            '''
            scheduler_1 = torch.optim.lr_scheduler.StepLR(optimizer=optimizer,step_size=20,gamma=0.5,last_epoch=-1)
...
if __name__ == '__main__':
    ...
    '''用第 3 类分类数据做相关性分析的结果
    ['滑油总管温度', '滑油压力调节阀开度指令', '滑油压力调节阀开度反馈', '2号电动滑油输送泵滑油出口压力', '滑油温度调节阀开度指令', 
    '滑油温度调节阀开度反馈', '滑油冷却器滑油进口温度', '汽轮主机减速器模拟装置滑油入口调节阀开度反馈', '滑油冷却器滑油出口温度', 
    '汽轮发电机组模拟装置滑油入口调节阀开度反馈', '汽轮主机模拟装置滑油出口温度', '汽轮主机减速器模拟装置调节阀阀门开度指令', 
    '汽轮主机减速器模拟装置调节阀开度反馈', '汽轮发电机组模拟装置调节阀开度反馈', '汽轮主机减速器模拟装置滑油出口温度',
     '汽轮给水泵组模拟装置调节阀开度反馈', '汽轮滑油泵组模拟装置调节阀开度反馈', '汽轮发电机组模拟装置滑油出口温度',
      '滑油加热器1号功率', '滑油加热器1号加热器出口温度', '滑油加热器2号功率', '滑油加热器2号加热器出口温度', 
      '主推力轴承模拟装置滑油出口温度', '汽轮凝水泵组模拟装置滑油出口温度', '汽轮给水泵组模拟装置滑油出口温度', 
      '汽轮滑油泵组模拟装置滑油出口温度', '汽轮辅循环水泵模拟装置滑油出口温度', '电动滑油泵2号变频泵转速反馈', 
      '滑油冷却器1号滑油入口压力', '滑油冷却器1号冷却水出口温度']
    '''
    predict_zhuansu("scan_data2.csv","电动滑油泵2号变频泵转速指令",corr_tp_set)